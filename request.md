# Advanced Crypto Technology Testnet - 先进加密货币技术示范平台
...
- **通信协议**：利用 mqtt 实现真实的 P2P 去中心化数据同步广播，实现跨网页的节点发现与数据同步。所有通信都是要经过远端传输。当多个使用者各自打开本网页，他们能够交换数据。接入点必须包括 'ws://test.mosquitto.org:8080'。
...
## 3. 界面与功能
### 3.1 初始页面
页面上方显示一段文字说明，提示使用者点击第一个按钮。
说明下方是一排四个按钮，依次是：建立网络、创建用户账户、创建区块链钞票、建立快速通道。后三个按钮为失活状态。
按钮区域下方是对象区域，有帮助、节点、账户、区块链、通道五个标签页。初始显示帮助页，内容为对本网页的介绍和使用说明，参照本文档。
### 3.2 建立网络
使用者点“建立网络”按钮之后：
...
用 d3.js 显示节点和彼此连接的情况，包括本网页上的节点和与本网页节点直接相连的外部节点。图必须与真实网络相符，不得造假。本网页节点与非本网页的节点颜色不同。
点击节点弹出此节点的详情页，其中有节点上的账户、相连的邻节点、收到的广播记录，还有广播测试区域。在广播记录中要显示完整的消息。
在广播测试区域输入一段消息并点“广播”按钮后，将此消息发送给所有邻节点，邻节点若是第一次收到这条消息，检查合法性通过后将它转发给其它邻节点。注意控流，不要造成无限转发。
检查合法性包括密码学检查和分叉检查。密码学检查包括验证签名，并检查签名的公钥与前一个支付区块中目标的公钥是否一致。分叉检查要检查同一位置是否已有其它区块，若发生分叉，将区块的添加者（公钥）加入黑名单。每个节点各自维护一个黑名单，收到黑名单中的用户签名的区块直接丢弃。
...
对象区域跳转到通道签页，显示已建立的通道，每个节点显示为一个方框，其中显示两个账户的余额。
...
## 5. AI 功能
在页面上，各功能模块和按钮旁边都显示一个小问号，点击弹出 AI 聊天窗口。每个问号对应一个默认问题，介绍对应功能的使用方法。弹出窗口后先回答默认问题，然后回答使用者后续的提问。
所有详情页的每项功能上也有同样的小问号。
回答首先导向本网页的功能和使用方法，以及 AOB 技术原理，特别突出与同类产品的差异。在使用者进一步追问时可增加技术实现细节。资料来源首先是本文件、help.md，也包括 AI 的内部资料。
AI 要根据页面当前的状态提示接下来做什么，回复要简短。
根据界面语言设定和用户输入选择 AI 使用的语言。
